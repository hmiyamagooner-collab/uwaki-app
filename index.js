...
const averageData = {
  男性: {
    '20代以下': { average: 38, topPercent: 22 },
    '30代':     { average: 44, topPercent: 18 },
    '40代':     { average: 47, topPercent: 15 },
    '50代以上': { average: 42, topPercent: 20 }
  },
  女性: {
    '20代以下': { average: 31, topPercent: 28 },
    '30代':     { average: 36, topPercent: 24 },
    '40代':     { average: 39, topPercent: 19 },
    '50代以上': { average: 34, topPercent: 26 }
  }
};
...
function brandHeader(rightText) {
  const contents = [
    { type: 'text', text: 'STEALTH FILE', size: 'xs', weight: 'bold', color: COLOR.gold, flex: 0 }
  ];
  if (rightText) {
    contents.push({ type: 'text', text: rightText, size: 'xs', color: COLOR.hintOnLight, align: 'end' });
  }
...
function createTopMessage() {
  return {
    ...
    hero: {
        ...
        action: { type: 'message', label: '診断をはじめる', text: '分析スタート' }
      },
      ...
      body: {
        ...
        paddingAll: '20px',
        contents: [
          { type: 'text', text: 'パートナー心理分析', size: 'lg', weight: 'bold', color: COLOR.textOnLight, align: 'center' },
          { type: 'text', text: 'その違和感、気のせいですか？\n15の質問で関係性をそっとチェック。', wrap: true, size: 'sm', color: COLOR.subOnLight, align: 'center', margin: 'sm' },
          {
            type: 'box', layout: 'vertical', margin: 'lg',
            contents: [
              { type: 'button', style: 'primary', color: COLOR.brown, height: 'md', action: { type: 'message', label: '診断をはじめる', text: '診断スタート' } }
            ]
          },
          { type: 'text', text: '匿名OK・記録は残りません', size: 'xs', color: COLOR.hintOnLight, align: 'center', margin: 'md' }
        ]
      }
    }
  };
...
function createGenderMessage() {
  return {
    ...
    contents: {
      ...
      body: {
        type: 'box', layout: 'vertical', spacing: 'md', paddingAll: '22px',
        contents: [
          brandHeader(''),
          { type: 'text', text: '性別を教えてください', size: 'md', weight: 'bold', color: COLOR.textOnLight, align: 'center', margin: 'md' },
          { type: 'text', text: 'より正確に分析するため、\n選択してください。', wrap: true, size: 'sm', color: COLOR.subOnLight, align: 'center', margin: 'sm' },
          {
            type: 'box', layout: 'vertical', spacing: 'sm', margin: 'lg',
            contents: GENDERS.map(g => ({
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          brandHeader(''),
          { type: 'text', text: '年代を教えてください', size: 'md', weight: 'bold', color: COLOR.textOnLight, align: 'center', margin: 'md' },
          { type: 'text', text: '現在の年代を選択してください。', wrap: true, size: 'sm', color: COLOR.subOnLight, align: 'center', margin: 'sm' },
          {
            type: 'box', layout: 'vertical', spacing: 'sm', margin: 'lg',
            contents: AGES.map(a => ({
              type: 'button', style: 'secondary', height: 'md',
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          {
            type: 'box', layout: 'vertical', height: '5px', backgroundColor: COLOR.trackLight, cornerRadius: '99px', margin: 'md',
            contents: [ { type: 'box', layout: 'vertical', width: `${Math.max(ratio,3)}%`, height: '5px', backgroundColor: COLOR.gold, cornerRadius: '99px', contents: [] } ]
          },
          { type: 'text', text: questionData.text, wrap: true, size: 'md', weight: 'bold', color: COLOR.textOnLight, align: 'center', margin: 'xl' },
          {
            type: 'box', layout: 'vertical', spacing: 'sm', margin: 'xl',
            contents: [
              { type: 'button', style: 'primary', color: COLOR.brown, height: 'md', action: { type: 'message', label: 'あてはまる', text: 'あてはまる' } },
              { type: 'button', style: 'secondary', height: 'md', action: { type: 'message', label: '少し気になる', text: '少し気になる' } },
              { type: 'button', style: 'secondary', height: 'md', action: { type: 'message', label: '気にならない', text: '気にならない' } }
            ]
          }
        ]
      }
      ...
      body: {
        ...
        contents: [
          ...
          { type: 'text', text: 'あなたの状況に合わせて、\n次の一歩をそっとご案内します。', wrap: true, size: 'sm', color: COLOR.subOnLight, align: 'center', margin: 'md' },
          {
            type: 'box', layout: 'vertical', spacing: 'sm', margin: 'xl',
            contents: [
              { type: 'button', style: 'primary', color: COLOR.brown, height: 'md', action: { type: 'message', label: '匿名でカウンセラーに相談', text: '相談したい' } },
              { type: 'button', style: 'secondary', height: 'md', action: { type: 'message', label: 'もう一度診断する', text: '診断スタート' } }
            ]
          },
...
function buildResult(current) {
  ...
  const std = (averageData[current.gender] && averageData[current.gender][current.age])
    ? averageData[current.gender][current.age]
    : { average: 40, topPercent: 20 };
  const diff = score - std.average;
  let compareText;
  if (diff >= 20) compareText = `同じ${current.age}の標準値より ${diff}pt 高めの傾向です。`;
  else if (diff >= 5) compareText = `標準値より ${diff}pt やや高めです。`;
  else if (diff >= -5) compareText = '標準値に近い傾向です。';
  else compareText = `標準値より ${Math.abs(diff)}pt 低めの傾向です。`;

  let rankText;
  if (diff >= 5) rankText = `注意シグナルが高めの方の傾向です（上位${std.topPercent}%目安）。`;
  else rankText = '気になる点はありつつ、落ち着いた傾向です。';

  let title, comment;
  if (score >= 80) { title = '関係性の状態：要セルフケア'; comment = '気になるシグナルが多く出ています。違和感は早めに整理しておくと、心の負担が軽くなります。'; }
  else if (score >= 60) { title = '関係性の状態：少し注意'; comment = '気になる変化がいくつか見られます。今の気持ちを、そのままにしすぎないことが大切です。'; }
  else if (score >= 40) { title = '関係性の状態：ゆらぎあり'; comment = '小さな違和感が見られます。焦らず、自分の感じ方を大切にしていきましょう。'; }
  else { title = '関係性の状態：おだやか'; comment = '今のところ、大きな揺らぎは見られませんでした。'; }

  const ageNote = {
    '20代以下': '20代は関係が動きやすい時期。気持ちの変化に敏感になりやすい年代です。',
    '30代':     '30代は仕事や環境の変化が多く、すれ違いが起きやすい時期です。',
    '40代':     '40代は生活リズムが安定する一方、小さな変化が見えにくくなることもあります。',
    '50代以上': '長く連れ添うほど、変化のサインは穏やかに表れることがあります。'
  }[current.age] || '';

  const counselorPatterns = {
    距離感: ['距離を感じる時間が増えたなら、関係の温度差を見つめ直すタイミングかもしれません。', '言葉よりも、態度の変化のほうが本音に近いことがあります。'],
    連絡の変化: ['「前と違う」という感覚は、あなたの心がすでに小さな変化を拾っているサインかもしれません。', '連絡の取り方の変化には、気持ちの優先順位が表れることがあります。'],
    生活リズム: ['生活リズムのズレが続くなら、理由の一貫性をそっと見てみてください。', '予定が読めなくなってきた時は、関係の透明度が少し下がっているのかもしれません。'],
    印象の変化: ['見た目や雰囲気の変化が重なる時は、慌てず様子を見るのが安心です。', '印象の変化そのものより、その理由が見えるかどうかが大切なポイントです。'],
    気持ちの揺れ: ['急な優しさや冷たさの落差には、心の揺れが表れていることがあります。', 'あなたが「前と違う」と感じているなら、その感覚は大切にしてあげてください。']
  };
  let topCat = RADAR_KEYS[0];
  for (const k of RADAR_KEYS) { if (analysis[k] > analysis[topCat]) topCat = k; }
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          { type: 'text', text: `${current.gender}・${current.age}`, size: 'xs', color: COLOR.subOnDark, align: 'center', margin: 'sm' },
          { type: 'text', text: 'あなたの関係性スコア', size: 'sm', color: COLOR.subOnDark, align: 'center', margin: 'md' },
          ...
          {
            type: 'box', layout: 'vertical', height: '8px', backgroundColor: COLOR.trackDark, cornerRadius: '99px', margin: 'lg',
            contents: [ { type: 'box', layout: 'vertical', width: `${Math.max(score,2)}%`, height: '8px', backgroundColor: COLOR.gold, cornerRadius: '99px', contents: [] } ]
          },
          ...
          {
            type: 'box', layout: 'vertical', backgroundColor: COLOR.cardOnDark, cornerRadius: 'md', paddingAll: '12px', margin: 'lg',
            contents: [
              { type: 'text', text: '📊 診断の標準値との比較', size: 'xs', weight: 'bold', color: COLOR.gold },
              { type: 'text', text: compareText, wrap: true, size: 'sm', color: COLOR.textOnDark, margin: 'sm' },
              { type: 'text', text: rankText, wrap: true, size: 'sm', color: COLOR.subOnDark, margin: 'sm' },
              { type: 'text', text: ageNote, wrap: true, size: 'xs', color: COLOR.subOnDark, margin: 'sm' }
            ]
          },
          {
            type: 'box', layout: 'vertical', backgroundColor: COLOR.cardOnDark, cornerRadius: 'md', paddingAll: '12px', margin: 'md',
            contents: [
              { type: 'text', text: '心理分析レーダー', size: 'xs', weight: 'bold', color: COLOR.gold },
              ...RADAR_KEYS.map(k => ({
                type: 'box', layout: 'horizontal', margin: 'sm',
                contents: [
                  { type: 'text', text: k, size: 'sm', color: COLOR.textOnDark, flex: 4 },
                  { type: 'text', text: bar(radar[k]), size: 'sm', color: COLOR.gold, align: 'end', flex: 5 }
                ]
              }))
            ]
          },
